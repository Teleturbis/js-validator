...
function isValidIban(val) {
    ...
    if (locale === 'de') {
        ...
        if (!asArray.filter(([key, value]) => (value.Bankleitzahl && value.Bankleitzahl === blz) ||
            (value['Nachfolge-Bankleitzahl'] &&
                value['Nachfolge-Bankleitzahl'] === blz)).length > 0)
    ...
    if (locale === 'at') {
        ...
        if (!asArray.filter(([key, value]) => value.Bankleitzahl && value.Bankleitzahl + '' === blz).length > 0)
