...
import { BlzAt, MessageObject } from '../../types/interfaces';

function isValidIban(val: string): MessageObject {
  ...
  const locale: string = val.substring(0, 2); // get locale
  ...
  if (locale === 'de') {
    ...
    const asArray: any[] = Object.entries(blzList.de); // convert object to array
    if (
      !(
        asArray.filter(
  ...
  if (locale === 'at') {
    ...
    bankInformation = asArray.filter(([key, value]: [key: any, value: any]) => {
  ...
  const checkValue = parseFloat(val.substring(2, 4)); // get check value
  ...
  const check = parseFloat(`${big[0] ? big[0] : 0}${big[1] ? big[1] : 0}`); // convert check value to string
